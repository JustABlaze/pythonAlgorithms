...
def createInitialPopulation(neededPop, lenOfChromo, xCons, yCons, function,
                            constArray):
    thisArray = []
    for i in range(0, neededPop):
        thisArray.append(
            gene(x=generateChromo(lenOfChromo), y=generateChromo(lenOfChromo)))
    thisArray = generateValues(
        array=thisArray,
        xCons=xCons,
        yCons=yCons,
...
def pickParents(array):
    ...
    while x2 == None:
        ...
        for i in range(0, len(arrayOfParents)):
            fit += arrayOfParents[(len(arrayOfParents) - 1) - i].fitness
            if rand < fit and x1 != arrayOfParents[(len(arrayOfParents) - 1)
                                                   - i]:
...
def mutation(array, mutationValue, lenOfChromo, xCons, yCons, function,
             constArray):
    for i in range(0, len(array)):
        rand = random.uniform(0, 1)
        if rand < mutationValue:
            randomMutation = round(random.uniform(0, lenOfChromo - 1))
            array[i].x[randomMutation] = 0 if array[i].x[
                randomMutation] == 1 else 1
            array[i].y[randomMutation] = 0 if array[i].y[
                randomMutation] == 1 else 1
...
def GA(x, y, population, lenOfChromo, generations, function, array):
    ...
    G_MAX = generations
    ps = numberOfPop // 2
    startingPop = createInitialPopulation(
        neededPop=numberOfPop,
        lenOfChromo=lenOfChromo,
        xCons=Xcon,
        yCons=Ycon,
        function=function,
...
array = [[4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4],
         [4, 2.1, 3, 4,
          4], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4], [
              4, 2.1, 3, 4, 4
          ], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4], [
              4, 2.1, 3, 4, 4
          ], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4], [
              4, 2.1, 3, 4, 4
          ], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4], [
              ...
              4, 2.1, 3, 4, 4
          ], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4,
                                 4], [4, 2.1, 3, 4, 4], [4, 2.1, 3, 4, 4],
...
for i in array:
    print(
        GA(x=3,
           ...
           lenOfChromo=11,
           generations=500,
           function=objectiveFuncparallel,
           array=i))
